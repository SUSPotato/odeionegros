...
namespace gamesense
{
    public partial class Form2 : Form
    {
        string HWID;
        // Vars
        string dll_link = ""; // Нажмите "Loader setup" для подробностей
        string hwid_link = ""; // Нажмите "Loader setup" для подробностей (Ссылка на файл с хвидами)
        ...
        // SETUP VAR
        bool setted_up = true; // Измените на true если вы закончили настройку loader'a
        // SETUP VAR
        ...
        private async void load_Click(object sender, EventArgs e)
        {
            ...
            if (access_list.Contains(HWID))
            {
                ...
                if (csgo_array.Length != 0)
                {
                    ...
                    Console.Read();
                    MessageBox.Show("Injected", "neverlose.cc", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    csgo.WaitForExit();
                    // Delete cheat
                ...
                {
                    MessageBox.Show("Error: CS:GO Process not found", "neverlose.cc", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Application.Exit();
                }
            }
            else
            {
                MessageBox.Show("Incorrect HWID", "neverlose.cc");
            }
        }
